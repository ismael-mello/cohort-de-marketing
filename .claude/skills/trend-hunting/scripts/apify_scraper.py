...
Esta skill usa o Apify pela API REST, não por MCP. O único pré-requisito e
...
Sem dependencias externas (so a stdlib). Sem token, nao quebra: imprime o
roteiro manual e sai.

Como funciona: usa o endpoint run-sync-get-dataset-items, que roda um Actor
do Apify e ja devolve os itens do dataset numa unica chamada.
...
Saida: JSON dos itens (lista) no stdout. O agente le e analisa.
...
def ler_token() -> str:
    ...
    if token:
        return token
    # tenta ler do .env na raiz (procura subindo ate achar)
    aqui = os.getcwd()
    for _ in range(6):
...
def roteiro_manual(motivo: str):
    print("=" * 64)
    print("MODO MANUAL (sem token Apify — a skill nao raspa sozinha)")
    print("=" * 64)
    print(f"\nMotivo: {motivo}\n")
    print("Configure o token uma vez:")
    print("  1. Crie a conta gratis: https://console.apify.com/sign-up")
...
def main():
    ...
    token = ler_token()
    if not token:
        roteiro_manual("APIFY_API_TOKEN nao encontrado no ambiente nem no .env")
        sys.exit(0)
    ...
    if args.modo == "instagram-hashtag":
        ...
        try:
            ...
            print(f"--input nao e JSON valido: {e}", file=sys.stderr)
            sys.exit(1)
